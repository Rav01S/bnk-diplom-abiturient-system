...
<div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-900"><strong>Комментарий сотрудника:</strong> {{ $application->rejection_reason }}</div>
@endif
...
<form method="POST" action="{{ route('applicant.applications.update', $application) }}" enctype="multipart/form-data" class="space-y-6" novalidate>
  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div><label for="study_form" class="block text-sm font-medium text-gray-700 mb-1">Форма обучения</label><select id="study_form" name="study_form" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-white"><option value="full_time" {{ old('study_form', $application->study_form)==='full_time'?'selected':'' }}>Очно</option>@if($application->program->has_study_form)<option value="part_time" {{ old('study_form', $application->study_form)==='part_time'?'selected':'' }}>Заочно</option>@endif</select>@unless($application->program->has_study_form)<p class="mt-1 text-xs text-blue-700">Для выбранной программы доступна только очная форма обучения.</p>@endunless</div>
