...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
  <form method="GET" class="flex flex-wrap gap-3 items-end">
    ...
    @if($selectedProgramId)<a href="{{ route('commission.ranking.export', ['program_id'=>$selectedProgramId, 'funding_type'=>$fundingType]) }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">📥 Распечатать список</a>@endif
  </form>
</div>
    ...
    <tbody class="divide-y divide-gray-200">
      ...
      <tr class="{{ ($i+1) <= $plan ? 'bg-blue-50' : '' }} hover:bg-gray-50">
        ...
        <td class="px-4 py-3 text-center">
          @php $boosted = $app->boostedCertScore(); @endphp
          @if($boosted !== null)
            <span class="inline-flex items-center text-primary-700 font-bold" title="{{ $app->hasPriorityBenefit() ? 'Вне конкурса' : 'Балл подменён настройкой льготы' }}">
              @if($app->hasPriorityBenefit())
                <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
              @endif
              {{ number_format($boosted, 2, ',', '') }} ({{ number_format($app->applicant->avg_cert_score ?? 0, 2, ',', '') }})
            </span>
          @else
