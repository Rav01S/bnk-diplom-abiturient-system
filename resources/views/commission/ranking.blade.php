...
@section('content')
<div class="mb-6"><h2 class="text-xl font-semibold text-gray-900">Рейтинговые списки</h2><p class="text-gray-500 text-sm">Одобренные заявления, отсортированные по сумме баллов</p></div>
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
  <form method="GET" class="flex flex-wrap gap-3 items-end">
    <div class="flex-1 min-w-[200px]"><label class="block text-xs font-medium text-gray-500 mb-1">Программа</label><select name="program_id" onchange="this.form.submit()" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white"><option value="">Выберите...</option>@foreach($programs as $p)<option value="{{ $p->id }}" {{ $selectedProgramId == $p->id ? 'selected' : '' }}>{{ $p->specialty->full_title }}</option>@endforeach</select></div>
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
  ...
  <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50"><tr><th class="px-4 py-3 text-left font-medium text-gray-500 w-12">№</th><th class="px-4 py-3 text-left font-medium text-gray-500">ФИО</th><th class="px-4 py-3 text-center font-medium text-gray-500">Балл 1</th><th class="px-4 py-3 text-center font-medium text-gray-500">Балл 2</th><th class="px-4 py-3 text-center font-medium text-gray-500">Балл 3</th><th class="px-4 py-3 text-center font-medium text-gray-500">Сумма</th><th class="px-4 py-3 text-center font-medium text-gray-500">Приор.</th></tr></thead>
    <tbody class="divide-y divide-gray-200">
      ...
      <tr class="{{ ($i+1) <= $plan ? 'bg-green-50' : '' }} hover:bg-gray-50">
        ...
        <td class="px-4 py-3 text-center font-bold">{{ $app->total_score }}</td>
