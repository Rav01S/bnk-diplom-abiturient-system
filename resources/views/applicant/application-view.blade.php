              ...
              <tr class="{{ $app->id === $application->id ? 'bg-primary-50 ring-1 ring-inset ring-primary-500' : '' }} hover:bg-gray-50">
                ...
                <td class="px-4 py-2.5">
                  <div class="flex items-center">
                    <span class="{{ $app->id === $application->id ? 'font-bold text-primary-700' : 'text-gray-900' }}">
                      {{ $app->app_full_name }}
                    </span>
                    @if($app->is_benefit && $app->benefit_type)
                      <span class="ml-1 text-[8px] font-bold px-1 py-0.5 bg-amber-100 text-amber-700 rounded uppercase" title="{{ $app->benefit_label }}">
                        Льгота
                      </span>
                    @endif
                  </div>
