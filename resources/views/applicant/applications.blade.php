      ...
      <tbody class="divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-50 transition">
          ...
          <td class="px-4 py-3 text-right space-x-1">
            ...
            <a href="{{ route('applicant.applications.ranking', $app) }}" target="_blank" class="text-primary-600 hover:text-primary-700 text-xs font-medium ml-1">Ранжирование</a>
            @if($app->isEditable())<a href="{{ route('applicant.applications.edit', $app) }}" class="text-primary-600 hover:text-primary-700 text-xs font-medium ml-1">Ред.</a>@endif
            @if($app->isCancellable())<form method="POST" action="{{ route('applicant.applications.cancel', $app) }}" class="inline" onsubmit="return confirm('Отменить заявление?')">@csrf @method('PATCH')<button type="submit" class="text-danger hover:text-blue-700 text-xs font-medium ml-1">Отменить</button></form>@endif
            @if($app->isDeletable())<form method="POST" action="{{ route('applicant.applications.destroy', $app) }}" class="inline" onsubmit="return confirm('Удалить заявление навсегда?')">@csrf @method('DELETE')<button type="submit" class="text-gray-500 hover:text-blue-700 text-xs font-medium ml-1">Удалить</button></form>@endif
          </td>
        </tr>
        @empty
        <tr><td colspan="8" class="px-4 py-8 text-center text-gray-500">Заявления не найдены</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
