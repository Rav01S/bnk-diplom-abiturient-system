  ...
  <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50/50"><tr><th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Год</th><th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Форма</th><th class="px-4 py-2 text-center text-xs font-medium text-gray-500">Бюджет</th><th class="px-4 py-2 text-center text-xs font-medium text-gray-500">Платно</th><th class="px-4 py-2 text-center text-xs font-medium text-gray-500">Статус</th><th class="px-4 py-2 text-center text-xs font-medium text-gray-500">Период</th><th class="px-4 py-2 text-right text-xs font-medium text-gray-500">Действия</th></tr></thead>
    <tbody class="divide-y divide-gray-100">
      ...
      <tr class="hover:bg-gray-50">
        ...
        <td class="px-4 py-2 text-xs text-gray-500">{{ $prog->has_study_form ? 'Очная/Заочная' : 'Только очная' }}</td>
        ...
        <td class="px-4 py-2 text-center">
          <form method="POST" action="{{ route('admin.programs.toggle', $prog) }}" class="inline">@csrf @method('PATCH')
            <button type="submit" class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $prog->is_open ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600' }}">
        ...
        <td class="px-4 py-2 text-right space-x-2">
          <button onclick="editProgramModal({
            ...
            has_form: {{ $prog->has_study_form ? 1 : 0 }},
  ...
  <div class="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
    ...
    <form id="programForm" method="POST" class="space-y-4">@csrf
      ...
      <div class="flex gap-6">
        <label class="flex items-center cursor-pointer"><input type="checkbox" name="has_study_form" value="1" checked class="w-4 h-4 text-primary-600 rounded mr-2"><span class="text-sm text-gray-700">Очная + Заочная формы</span></label>
...
function openProgramModal(specId, title) {
  ...
  form.querySelector('[name="has_study_form"]').checked = true;
...
function editProgramModal(prog, specTitle) {
  ...
  form.querySelector('[name="has_study_form"]').checked = !!prog.has_form;
